...
if(isset($_POST) && array_key_exists('cbutton',$_POST))
{
   ...
   $query="select * from adminsignin";
   $query1="update adminsignin set Password='$npswd' where Adminname='$usr'";
   ...
   while($result=mysql_fetch_array($inp))
    {
       if($result['Adminname']==$usr)
       {  
         $stat=1;
          $inp1=mysql_query($query1,$con);
          if(!$inp1)
           header("Location: aresetpas.php?message=Password didin't update Successfully. Please try again later.");
          else
           header("Location: aresetpas.php?message1=Password updated successfully. Login with new credentials.");  
...
header("refresh : 120; url=adminsignin.php");
?>
